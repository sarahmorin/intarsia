...
fn main() {
    ...
    println!("=== Kymetica Query Optimizer Demo ===\n");
    ...
    // ============================================================
    // Step 3: Initialize Optimizer Context
    // ============================================================

    println!("Step 3: Initializing optimizer context...");
    let mut ctx = OptimizerContext::new(catalog);

    // Add the initial expression to the e-graph
    let root_id = ctx.init(initial_expr);

    println!("  ✓ Initial expression added to e-graph");
    println!(
        "  ✓ E-graph initialized with {} nodes\n",
        ctx.egraph.total_number_of_nodes()
    );

    // Extract initial plan before optimization
    let (initial_cost, initial_plan) = ctx.clone().extract_with_cost(root_id);
    println!("Initial Plan (before optimization):");
    println!("  Expression: {}", initial_plan);
    println!("  Cost: {:?}\n", initial_cost);
    ...
    let (optimized_cost, optimized_plan) = ctx.extract_with_cost(root_id);
