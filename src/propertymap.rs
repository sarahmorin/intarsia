...
pub struct PropertySetMap<P: PropertySet>(BiMap<P, usize>);
    ...
    P: PropertySet,
{
    ...
    pub fn insert(&mut self, prop: &P) -> usize {
    ...
    pub fn get_by_id(&self, id: &usize) -> Option<&P> {
        self.0.get_by_right(id)
    }

    /// Retrieves the unique ID for a given property set, if it exists.
    pub fn get_by_propset(&self, prop_set: &P) -> Option<&usize> {
...
mod tests {
    ...
    impl PropertySet for TestPropertySet {
        fn bottom() -> Self {
            TestPropertySet { value: 0 }
        }
    }

    impl TestPropertySet {
        fn new(value: usize) -> Self {
            TestPropertySet { value }
        }
    }
    ...
    fn test_new_property_map() {
        ...
        assert_eq!(prop_map.get_by_id(&0), Some(&TestPropertySet::bottom()));
        assert_eq!(prop_map.get_by_propset(&TestPropertySet::bottom()), Some(&0));
    }
