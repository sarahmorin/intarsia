use egg::{
    Analysis, DidMerge, Id, Language, Pattern, PatternAst, Subst, Symbol, define_language,
    merge_option, rewrite as rw,
};
use ordered_float::NotNan;

pub type EGraph = egg::EGraph<Math, ConstantFold>;
pub type MathRewrite = egg::Rewrite<Math, ConstantFold>;
...
#[rustfmt::skip]
pub fn rules() -> Vec<MathRewrite> { vec![
    rw!("comm-add";  "(+ ?a ?b)"        => "(+ ?b ?a)"),
    rw!("comm-mul";  "(* ?a ?b)"        => "(* ?b ?a)"),
...
// Default configuration values used by both benchmark engines.
// Environment variables:
// - EGG_ITER_LIMIT defaults to DEFAULT_EGG_ITER_LIMIT
// - EGG_NODE_LIMIT defaults to DEFAULT_NODE_LIMIT
// - EGG_TIME_LIMIT defaults to DEFAULT_TIME_LIMIT_S
// - INTARSIA_TASK_LIMIT defaults to DEFAULT_INTARSIA_TASK_LIMIT
// - INTARSIA_NODE_LIMIT defaults to DEFAULT_NODE_LIMIT
// - INTARSIA_TIME_LIMIT defaults to DEFAULT_TIME_LIMIT_S
pub const DEFAULT_NODE_LIMIT: usize = 1_000_000;
pub const DEFAULT_TIME_LIMIT_S: u64 = 1000;
pub const DEFAULT_EGG_ITER_LIMIT: usize = 50;
pub const DEFAULT_INTARSIA_TASK_LIMIT: usize = 500_000;
...
pub fn env_var<T: std::str::FromStr>(name: &str, default: T) -> T {
    std::env::var(name)
        .ok()
...
#[allow(dead_code)]
pub fn percentile(k: f64, data: &[u128]) -> u128 {
    assert!((0.0..=1.0).contains(&k));
    let i = (data.len() as f64 * k) as usize;
    data[i.min(data.len() - 1)]
}

#[allow(dead_code)]
pub fn build_patterns(rules: &[MathRewrite], extra_patterns: &[&str]) -> Vec<Pattern<Math>> {
    let mut patterns: Vec<Pattern<Math>> = vec![];
    for rule in rules {
    ...
    for extra in extra_patterns {
        let p: Pattern<Math> = extra.parse().unwrap();
