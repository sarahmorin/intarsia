...
define_language! {
    pub enum RewriteLang {
        // Logical operations
        Source(i64), // TODO: How can we encode literals within a node?
        "OP1" = Op1(Id),
        "OP2" = Op2([Id; 2]),
        "NET" = Net(Id),
    }
...
impl PropertyAwareLanguage<NoProperty> for RewriteLang {
...
pub type RewriteOptimizer = SimpleOptimizerFramework<RewriteLang, NoProperty>;

// 5. Implement the Context trait for our optimizer to link to ISLE ruleset.
impl Context for RewriteOptimizer {
    ...
    fn extractor_source(&mut self, arg0: Id) -> Option<i64> {
        ...
        match node {
            RewriteLang::Source(x) => Some(*x),
            _ => None,
        }
    }

    fn constructor_source(&mut self, arg0: i64) -> Id {
        let node = RewriteLang::Source(arg0);
        let (id, _) = self.egraph.add_with_flag(node);
        id
    }

    isle_accessors! {
        RewriteLang::Op1(extractor_op1, constructor_op1, 1);
        RewriteLang::Op2(extractor_op2, constructor_op2, 2);
        RewriteLang::Net(extractor_net, constructor_net, 1);
    }
}

// 6. Implement ExplorerHooks to link to the ISLE ruleset entrypoint.
impl ExplorerHooks<RewriteLang> for RewriteOptimizer {
